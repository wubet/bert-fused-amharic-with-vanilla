...
from fairseq.dataclass.configs import CommonConfig, DatasetConfig, DistributedTrainingConfig, OptimizationConfig, \
    CheckpointConfig, EMAConfig
...
def get_training_parser(default_task='translation', is_vanilla=False):
    if is_vanilla:
        parser = get_vanilla_parser('Trainer', default_task)
        add_vanilla_dataset_args(parser, train=True)
        add_vanilla_distributed_training_args(parser)
        add_model_args(parser)
        add_vanilla_optimization_args(parser)
...
def get_parser(desc, default_task='translation'):
    ...
    utils_bert.import_user_module(usr_args)
...
def get_vanilla_parser(desc, default_task="translation"):
    # Before creating the true parser, we need to import optional user module
    # in order to eagerly import custom tasks, optimizers, architectures, etc.
    usr_parser = argparse.ArgumentParser(add_help=False, allow_abbrev=False)
    usr_parser.add_argument("--user-dir", default=None)
...
def add_vanilla_dataset_args(parser, train=False, gen=False):
...
def add_vanilla_distributed_training_args(parser, default_world_size=None):
...
def add_vanilla_optimization_args(parser):
    group = parser.add_argument_group("optimization")
    # fmt: off
...
def add_vanilla_checkpoint_args(parser):
