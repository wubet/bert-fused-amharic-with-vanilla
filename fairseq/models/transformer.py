...
class TransformerModel(FairseqEncoderDecoderModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--activation-fn',
                            choices=utils_bert.get_available_activation_fns(),
    ...
    def build_model(cls, args, task):
        ...
        def build_embedding(dictionary, embed_dim, path=None):
            ...
            if path:
                embed_dict = utils_bert.parse_embedding(path)
                utils_bert.load_embedding(embed_dict, dictionary, emb)
            return emb
...
class TransformerEncoder(FairseqEncoder):
    ...
    def upgrade_state_dict_named(self, state_dict, name):
        ...
        if utils_bert.item(state_dict.get(version_key, torch.Tensor([1]))[0]) < 2:
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def buffered_future_mask(self, tensor):
        ...
        if not hasattr(self, '_future_mask') or self._future_mask is None or self._future_mask.device != tensor.device:
            self._future_mask = torch.triu(utils_bert.fill_with_neg_inf(tensor.new(dim, dim)), 1)
        if self._future_mask.size(0) < dim:
            self._future_mask = torch.triu(utils_bert.fill_with_neg_inf(self._future_mask.resize_(dim, dim)), 1)
...
class TransformerEncoderLayer(nn.Module):
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def __init__(self, args):
        ...
        self.dropout = args.dropout
        self.activation_fn = utils_bert.get_activation_fn(
...
# @register_model_architecture("transformer", "transformer_wmt_van_en_am")
# def transformer_wmt_van_en_am(args):
#     base_architecture(args)
