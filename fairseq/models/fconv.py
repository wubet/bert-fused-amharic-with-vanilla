...
class FConvModel(FairseqEncoderDecoderModel):
    ...
    def build_model(cls, args, task):
        ...
        if args.encoder_embed_path:
            encoder_embed_dict = utils_bert.parse_embedding(args.encoder_embed_path)
            utils_bert.print_embed_overlap(encoder_embed_dict, task.source_dictionary)

        decoder_embed_dict = None
        if args.decoder_embed_path:
            decoder_embed_dict = utils_bert.parse_embedding(args.decoder_embed_path)
            utils_bert.print_embed_overlap(decoder_embed_dict, task.target_dictionary)
...
class FConvEncoder(FairseqEncoder):
    ...
    def __init__(
        ...
        if embed_dict:
            self.embed_tokens = utils_bert.load_embedding(embed_dict, self.dictionary, self.embed_tokens)
...
class FConvDecoder(FairseqIncrementalDecoder):
    ...
    def reorder_incremental_state(self, incremental_state, new_order):
        ...
        encoder_out = utils_bert.get_incremental_state(self, incremental_state, 'encoder_out')
        if encoder_out is not None:
            encoder_out = tuple(eo.index_select(0, new_order) for eo in encoder_out)
            utils_bert.set_incremental_state(self, incremental_state, 'encoder_out', encoder_out)
    ...
    def upgrade_state_dict(self, state_dict):
        if utils_bert.item(state_dict.get('decoder.version', torch.Tensor([1]))[0]) < 2:
    ...
    def _split_encoder_out(self, encoder_out, incremental_state):
        ...
        """
        cached_result = utils_bert.get_incremental_state(self, incremental_state, 'encoder_out')
        ...
        if incremental_state is not None:
            utils_bert.set_incremental_state(self, incremental_state, 'encoder_out', result)
        return result
