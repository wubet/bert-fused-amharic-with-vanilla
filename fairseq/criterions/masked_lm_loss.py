...
class MaskedLmLoss(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        ntokens = utils_bert.strip_pad(lm_targets, self.padding_idx).numel()
        ...
        logging_output = {
            ...
            'lm_loss': utils_bert.item(lm_loss.data) if reduce else lm_loss.data,
            # sentence loss is not always computed
            'sentence_loss': (
                (
                    utils_bert.item(sentence_loss.data) if reduce
