...
from fairseq import checkpoint_utils, distributed_utils, models, optim, utils_bert
...
class Trainer(object):
    ...
    def train_step(self, samples, dummy_batch=False, raise_oom=False):
        ...
        for i, sample in enumerate(samples):
            ...
            try:
                ...
                loss, sample_size, logging_output = self.task.train_bert_step(
    ...
    def _prepare_sample(self, sample):
        if sample is None or len(sample) == 0:
            return None
        if self.cuda:
            sample = utils_bert.move_to_cuda(sample)
        return sample
